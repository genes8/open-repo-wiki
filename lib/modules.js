...
const SEMANTIC_MODULE_TITLES = Object.freeze({
  app: 'Application',
  apps: 'Applications',
...
function semanticModuleTitle(name) {
  const normalized = String(name).replace(/\\/g, '/').split('/').filter(Boolean).at(-1) || '';
  return SEMANTIC_MODULE_TITLES[normalized.toLowerCase()] || titleCase(normalized);
}
