...
function safeManagedPath(baseDir, relativePath) {
  ...
  if (fs.existsSync(base) && fs.lstatSync(base).isSymbolicLink()) return null;
  let parent = path.dirname(full);
  while (parent !== base) {
    if (!parent.startsWith(`${base}${path.sep}`)) return null;
...
function writeManifest(baseDir, files) {
  ...
  const resolved = safeManagedPath(baseDir, '_manifest.json');
  if (!resolved) throw new Error('unsafe knowledge manifest path');
...
function validateKnowledgeContent(content) {
  ...
  if (hasRefusalText(text)) {
    ...
    violations.push({
      ...
      message: 'knowledge card is empty',
    });
  } else if (text !== '_Not applicable for this module._' && text.length < 50) {
    violations.push({
